...
function converter(options) {
  ...
  var startTime = Date.now() / 1000 // start time in seconds

  tapParser.on('line', function(line) {
    // still want normal tap output
    console.error(line)
  })
  ...
  tapParser.on('complete', function(r) {
    ...
    var totalTime = (Date.now() / 1000 - startTime).toFixed(2)
    ...
    if (tapParser.sawValidTap && planPresent) {
      var xmlString = serialize(testSuites, totalTime);
